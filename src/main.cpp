...
Mesh parseObj(const char *path, double &reindex) {
    ...
    size_t total_vertices = meshopt_generateVertexRemap(
        &remap[0], NULL, total_indices, &vertices[0], total_indices, sizeof(Vertex));

    result.indices.resize(total_indices);
    meshopt_remapIndexBuffer(&result.indices[0], NULL, total_indices, &remap[0]);

    result.vertices.resize(total_vertices);
    meshopt_remapVertexBuffer(&result.vertices[0], &vertices[0], total_indices, sizeof(Vertex),
                              &remap[0]);

    return result;
}

void dumpObj(const std::vector<Vertex> &vertices, const std::vector<unsigned int> &indices,
...
void simplify(const Mesh &mesh, float threshold = 0.2f, unsigned int options = 0) {
    ...
    lod.indices.resize(mesh.indices.size()); // note: simplify needs space for index_count elements
                                             // in the destination array, not target_index_count
    lod.indices.resize(meshopt_simplify(&lod.indices[0], &mesh.indices[0], mesh.indices.size(),
                                        &mesh.vertices[0].px, mesh.vertices.size(), sizeof(Vertex),
                                        target_index_count, target_error, options, &result_error));

    lod.vertices.resize(
        lod.indices.size() < mesh.vertices.size()
            ? lod.indices.size()
            : mesh.vertices.size()); // note: this is just to reduce the cost of resize()
    lod.vertices.resize(meshopt_optimizeVertexFetch(&lod.vertices[0], &lod.indices[0],
                                                    lod.indices.size(), &mesh.vertices[0],
                                                    mesh.vertices.size(), sizeof(Vertex)));

    double end = timestamp();

    printf("%-9s: %d triangles => %d triangles (%.2f%% deviation) in %.2f msec\n", "Simplify",
           int(mesh.indices.size() / 3), int(lod.indices.size() / 3), result_error * 100,
           (end - start) * 1000);
}
...
void simplifyAttr(const Mesh &mesh, float threshold = 0.2f, unsigned int options = 0) {
    ...
    lod.indices.resize(meshopt_simplifyWithAttributes(
        &lod.indices[0], &mesh.indices[0], mesh.indices.size(), &mesh.vertices[0].px,
        mesh.vertices.size(), sizeof(Vertex), &mesh.vertices[0].nx, sizeof(Vertex), attr_weights, 3,
        NULL, target_index_count, target_error, options, &result_error));
...
void simplifyUpdate(const Mesh &mesh, float threshold = 0.2f, unsigned int options = 0) {
    ...
    lod.indices.resize(meshopt_simplifyWithUpdate(
        &lod.indices[0], mesh.indices.size(), &lod.vertices[0].px, mesh.vertices.size(),
        sizeof(Vertex), &lod.vertices[0].nx, sizeof(Vertex), attr_weights, 3, NULL,
        target_index_count, target_error, options, &result_error));
...
void simplifySloppy(const Mesh &mesh, float threshold = 0.2f) {
    ...
    lod.indices.resize(meshopt_simplifySloppy(
        &lod.indices[0], &mesh.indices[0], mesh.indices.size(), &mesh.vertices[0].px,
        mesh.vertices.size(), sizeof(Vertex), target_index_count, target_error, &result_error));
...
void simplifyPoints(const Mesh &mesh, float threshold = 0.2f) {
    ...
    indices.resize(meshopt_simplifyPoints(&indices[0], &mesh.vertices[0].px, mesh.vertices.size(),
                                          sizeof(Vertex), NULL, 0, 0, target_vertex_count));

    double end = timestamp();

    printf("%-9s: %d points => %d points in %.2f msec\n", "SimplifyP", int(mesh.vertices.size()),
           int(indices.size()), (end - start) * 1000);
}
...
void simplifyComplete(const Mesh &mesh) {
    ...
    for (size_t i = 1; i < lod_count; ++i) {
        ...
        // simplifying from the base level sometimes produces better results, but simplifying from
        // last level is faster
        ...
        lod.resize(meshopt_simplify(&lod[0], &source[0], source.size(), &mesh.vertices[0].px,
                                    mesh.vertices.size(), sizeof(Vertex), target_index_count,
                                    target_error));
    }
    ...
    for (size_t i = 0; i < lod_count; ++i) {
        ...
        meshopt_optimizeOverdraw(&lod[0], &lod[0], lod.size(), &mesh.vertices[0].px,
                                 mesh.vertices.size(), sizeof(Vertex), 1.0f);
    }

    // concatenate all LODs into one IB
    // note: the order of concatenation is important - since we optimize the entire IB for vertex
    // fetch, putting coarse LODs first makes sure that the vertex range referenced by them is as
    // small as possible some GPUs process the entire range referenced by the index buffer region so
    // doing this optimizes the vertex transform cost for coarse LODs this order also produces much
    // better vertex fetch cache coherency for coarse LODs (since they're essentially optimized
    // first) somewhat surprisingly, the vertex fetch cache coherency for fine LODs doesn't seem to
    // suffer that much.
    ...
    meshopt_optimizeVertexFetch(&vertices[0], &indices[0], indices.size(), &vertices[0],
                                vertices.size(), sizeof(Vertex));

    double end = timestamp();

    printf("%-9s: %d triangles => %d LOD levels down to %d triangles in %.2f msec, optimized in "
           "%.2f msec\n",
           "SimplifyC", int(lod_index_counts[0]) / 3, int(lod_count),
           int(lod_index_counts[lod_count - 1]) / 3, (middle - start) * 1000,
           (end - middle) * 1000);
...

    {
        meshopt_VertexCacheStatistics vcs0 = meshopt_analyzeVertexCache(
            &indices[lod_index_offsets[0]], lod_index_counts[0], vertices.size(), 16, 0, 0);
        ...
        meshopt_VertexCacheStatistics vcsN =
            meshopt_analyzeVertexCache(&indices[lod_index_offsets[lod_count - 1]],
                                       lod_index_counts[lod_count - 1], vertices.size(), 16, 0, 0);
        meshopt_VertexFetchStatistics vfsN = meshopt_analyzeVertexFetch(
            &indices[lod_index_offsets[lod_count - 1]], lod_index_counts[lod_count - 1],
            vertices.size(), sizeof(Vertex));
        ...
        std::vector<unsigned char> vbuf(
            meshopt_encodeVertexBufferBound(vertices.size(), sizeof(PV)));
        vbuf.resize(
            meshopt_encodeVertexBuffer(&vbuf[0], vbuf.size(), &pv[0], vertices.size(), sizeof(PV)));

        std::vector<unsigned char> ibuf(
            meshopt_encodeIndexBufferBound(indices.size(), vertices.size()));
        ibuf.resize(meshopt_encodeIndexBuffer(&ibuf[0], ibuf.size(), &indices[0], indices.size()));

        printf(
            "%-9s  ACMR %f...%f Overfetch %f..%f Codec VB %.1f bits/vertex IB %.1f bits/triangle\n",
...
void simplifyClusters(const Mesh &mesh, float threshold = 0.2f) {
    ...
    meshlets.resize(meshopt_buildMeshlets(&meshlets[0], &meshlet_vertices[0], &meshlet_triangles[0],
                                          &mesh.indices[0], mesh.indices.size(),
                                          &mesh.vertices[0].px, mesh.vertices.size(),
                                          sizeof(Vertex), max_vertices, max_triangles, 0.f));
    ...
    meshopt_generatePositionRemap(&remap[0], &mesh.vertices[0].px, mesh.vertices.size(),
                                  sizeof(Vertex));

    // partition clusters in groups; each group will be simplified separately and the boundaries
    // between groups will be preserved
    std::vector<unsigned int> cluster_indices;
    cluster_indices.reserve(
        mesh.indices.size()); // slight underestimate, vector should realloc once
    ...
    for (size_t i = 0; i < meshlets.size(); ++i) {
        ...
        for (size_t j = 0; j < m.triangle_count * 3; ++j) {
            unsigned int v =
                meshlet_vertices[m.vertex_offset + meshlet_triangles[m.triangle_offset + j]];

            // use the first vertex with equivalent position so that cluster adjacency ignores
            // attribute seams
    ...
    size_t partition_count =
        meshopt_partitionClusters(&partition[0], &cluster_indices[0], cluster_indices.size(),
                                  &cluster_sizes[0], cluster_sizes.size(), &mesh.vertices[0].px,
                                  mesh.vertices.size(), sizeof(Vertex), target_group_size);

    // convert partitions to linked lists to make it easier to iterate over (vectors of vectors
    // would work too)
    ...
    for (size_t i = 0; i < meshlets.size(); ++i) {
        ...
        for (int j = int(i); j >= 0; j = partnext[j]) {
            ...
            for (size_t k = 0; k < m.triangle_count * 3; ++k)
                lod.push_back(
                    meshlet_vertices[m.vertex_offset + meshlet_triangles[m.triangle_offset + k]]);
        }

        size_t group_triangles = (lod.size() - group_offset) / 3;

        // simplify the group, preserving the border vertices
        // note: this technically also locks the exterior border; a full mesh analysis (see
        // clusterlod.h / lockBoundary) would work better for some meshes
        ...
        size_t group_size =
            meshopt_simplify(&lod[group_offset], &lod[group_offset], group_triangles * 3,
                             &mesh.vertices[0].px, mesh.vertices.size(), sizeof(Vertex),
                             group_target, group_target_error, options, &group_error);
    ...
    printf("%-9s: %d triangles => %d triangles (%.2f%% deviation) in %.2f msec, clusterized in "
           "%.2f msec, partitioned in %.2f msec (%d clusters in %d groups, %.1f avg)\n",
           "SimplifyG", int(mesh.indices.size() / 3), int(lod.size() / 3), error / scale * 100,
           (end - parttime) * 1000, (middle - start) * 1000, (parttime - middle) * 1000,
           int(meshlets.size()), int(partition_count),
           double(meshlets.size()) / double(partition_count));
}

void optimize(const Mesh &mesh, bool fifo = false) {
    Mesh copy = mesh;
    // note: we assume that the mesh is already optimally indexed (via parseObj); if that is not the
    // case, you'd need to reindex first

    double start = timestamp();

    // vertex cache optimization should go first as it provides starting order for overdraw
    // note: fifo optimization is not recommended as a default, since it produces worse results, but
    // it's faster to run so it can be useful for procedural meshes
    if (fifo)
        meshopt_optimizeVertexCacheFifo(&copy.indices[0], &copy.indices[0], copy.indices.size(),
                                        copy.vertices.size(), /* cache_size= */ 16);
    else
        meshopt_optimizeVertexCache(&copy.indices[0], &copy.indices[0], copy.indices.size(),
                                    copy.vertices.size());

    // reorder indices for overdraw, balancing overdraw and vertex cache efficiency
    const float kThreshold =
        1.01f; // allow up to 1% worse ACMR to get more reordering opportunities for overdraw
    meshopt_optimizeOverdraw(&copy.indices[0], &copy.indices[0], copy.indices.size(),
                             &copy.vertices[0].px, copy.vertices.size(), sizeof(Vertex),
                             kThreshold);

    // vertex fetch optimization should go last as it depends on the final index order
    meshopt_optimizeVertexFetch(&copy.vertices[0], &copy.indices[0], copy.indices.size(),
                                &copy.vertices[0], copy.vertices.size(), sizeof(Vertex));

    double end = timestamp();

    meshopt_VertexCacheStatistics vcs = meshopt_analyzeVertexCache(
        &copy.indices[0], copy.indices.size(), copy.vertices.size(), 16, 0, 0);
    meshopt_VertexFetchStatistics vfs = meshopt_analyzeVertexFetch(
        &copy.indices[0], copy.indices.size(), copy.vertices.size(), sizeof(Vertex));
    meshopt_OverdrawStatistics os =
        meshopt_analyzeOverdraw(&copy.indices[0], copy.indices.size(), &copy.vertices[0].px,
                                copy.vertices.size(), sizeof(Vertex));

    meshopt_VertexCacheStatistics vcs_nv = meshopt_analyzeVertexCache(
        &copy.indices[0], copy.indices.size(), copy.vertices.size(), 32, 32, 32);
    meshopt_VertexCacheStatistics vcs_amd = meshopt_analyzeVertexCache(
        &copy.indices[0], copy.indices.size(), copy.vertices.size(), 14, 64, 128);
    meshopt_VertexCacheStatistics vcs_intel = meshopt_analyzeVertexCache(
        &copy.indices[0], copy.indices.size(), copy.vertices.size(), 128, 0, 0);

    printf("Optimize%s: ACMR %f ATVR %f (NV %f AMD %f Intel %f) overfetch %f overdraw %f in %.2f "
           "msec\n",
           fifo ? "F" : " ", vcs.acmr, vcs.atvr, vcs_nv.atvr, vcs_amd.atvr, vcs_intel.atvr,
           vfs.overfetch, os.overdraw, (end - start) * 1000);
}

template <typename T> size_t compress(const std::vector<T> &data, int level = SDEFL_LVL_DEF) {
    std::vector<unsigned char> cbuf(sdefl_bound(int(data.size() * sizeof(T))));
    sdefl s = {};
    return sdeflate(&s, &cbuf[0], reinterpret_cast<const unsigned char *>(&data[0]),
                    int(data.size() * sizeof(T)), level);
}
...
void encodeIndex(const std::vector<unsigned int> &indices, size_t vertex_count, char desc) {
    ...
    buffer.resize(
        meshopt_encodeIndexBuffer(&buffer[0], buffer.size(), &indices[0], indices.size()));
    ...
    for (size_t i = 0; i < indices.size(); i += 3) {
        assert((result[i + 0] == indices[i + 0] && result[i + 1] == indices[i + 1] &&
                result[i + 2] == indices[i + 2]) ||
               (result[i + 1] == indices[i + 0] && result[i + 2] == indices[i + 1] &&
                result[i + 0] == indices[i + 2]) ||
               (result[i + 2] == indices[i + 0] && result[i + 0] == indices[i + 1] &&
                result[i + 1] == indices[i + 2]));
    }

    printf("IdxCodec%c: %.1f bits/triangle (post-deflate %.1f bits/triangle); encode %.2f msec "
           "(%.3f GB/s), decode %.2f msec (%.2f GB/s)\n",
           desc, double(buffer.size() * 8) / double(indices.size() / 3),
           double(csize * 8) / double(indices.size() / 3), (middle - start) * 1000,
           (double(result.size() * 4) / 1e9) / (middle - start), (end - middle) * 1000,
...
void encodeIndexSequence(const std::vector<unsigned int> &data, size_t vertex_count, char desc) {
    ...
    printf("IdxCodec%c: %.1f bits/index (post-deflate %.1f bits/index); encode %.2f msec (%.3f "
           "GB/s), decode %.2f msec (%.2f GB/s)\n",
           desc, double(buffer.size() * 8) / double(data.size()),
           double(csize * 8) / double(data.size()), (middle - start) * 1000,
           (double(result.size() * 4) / 1e9) / (middle - start), (end - middle) * 1000,
           (double(result.size() * 4) / 1e9) / (end - middle));
}
...
template <typename PV> void encodeVertex(const Mesh &mesh, const char *pvn, int level = 2) {
    std::vector<PV> pv(mesh.vertices.size());
    packMesh(pv, mesh.vertices);
    ...
    std::vector<unsigned char> vbuf(
        meshopt_encodeVertexBufferBound(mesh.vertices.size(), sizeof(PV)));
    vbuf.resize(meshopt_encodeVertexBufferLevel(&vbuf[0], vbuf.size(), &pv[0], mesh.vertices.size(),
                                                sizeof(PV), level, -1));

    double middle = timestamp();

    int res = meshopt_decodeVertexBuffer(&result[0], mesh.vertices.size(), sizeof(PV), &vbuf[0],
                                         vbuf.size());
    assert(res == 0);
    (void)res;
    ...
    printf("VtxCodec%1s: %.1f bits/vertex (post-deflate %.1f bits/vertex); encode %.2f msec (%.3f "
           "GB/s), decode %.2f msec (%.2f GB/s)\n",
           pvn, double(vbuf.size() * 8) / double(mesh.vertices.size()),
           double(csize * 8) / double(mesh.vertices.size()), (middle - start) * 1000,
           (double(result.size() * sizeof(PV)) / 1e9) / (middle - start), (end - middle) * 1000,
...
void stripify(const Mesh &mesh, bool use_restart, char desc) {
    ...
    strip.resize(meshopt_stripify(&strip[0], &mesh.indices[0], mesh.indices.size(),
                                  mesh.vertices.size(), restart_index));
    double end = timestamp();
    ...
    Mesh copy = mesh;
    copy.indices.resize(
        meshopt_unstripify(&copy.indices[0], &strip[0], strip.size(), restart_index));
    assert(copy.indices.size() <= meshopt_unstripifyBound(strip.size()));

    meshopt_VertexCacheStatistics vcs = meshopt_analyzeVertexCache(
        &copy.indices[0], mesh.indices.size(), mesh.vertices.size(), 16, 0, 0);
    meshopt_VertexCacheStatistics vcs_nv = meshopt_analyzeVertexCache(
        &copy.indices[0], mesh.indices.size(), mesh.vertices.size(), 32, 32, 32);
    meshopt_VertexCacheStatistics vcs_amd = meshopt_analyzeVertexCache(
        &copy.indices[0], mesh.indices.size(), mesh.vertices.size(), 14, 64, 128);
    meshopt_VertexCacheStatistics vcs_intel = meshopt_analyzeVertexCache(
        &copy.indices[0], mesh.indices.size(), mesh.vertices.size(), 128, 0, 0);

    printf("Stripify%c: ACMR %f ATVR %f (NV %f AMD %f Intel %f); %.1f run avg, %d strip indices "
           "(%.1f%%) in %.2f msec\n",
           desc, vcs.acmr, vcs.atvr, vcs_nv.atvr, vcs_amd.atvr, vcs_intel.atvr,
...
void shadow(const Mesh &mesh) {
    ...
    double start = timestamp();
    // this index buffer can be used for position-only rendering using the same vertex data that the
    // original index buffer uses
    std::vector<unsigned int> shadow_indices(mesh.indices.size());
    meshopt_generateShadowIndexBuffer(&shadow_indices[0], &mesh.indices[0], mesh.indices.size(),
                                      &mesh.vertices[0], mesh.vertices.size(), sizeof(float) * 3,
                                      sizeof(Vertex));
    double end = timestamp();

    // while you can't optimize the vertex data after shadow IB was constructed, you can and should
    // optimize the shadow IB for vertex cache this is valuable even if the original indices array
    // was optimized for vertex cache!
    meshopt_optimizeVertexCache(&shadow_indices[0], &shadow_indices[0], shadow_indices.size(),
                                mesh.vertices.size());

    meshopt_VertexCacheStatistics vcs = meshopt_analyzeVertexCache(
        &mesh.indices[0], mesh.indices.size(), mesh.vertices.size(), 16, 0, 0);
    meshopt_VertexCacheStatistics vcss = meshopt_analyzeVertexCache(
        &shadow_indices[0], shadow_indices.size(), mesh.vertices.size(), 16, 0, 0);
    ...
    printf("ShadowIB : ACMR %f (%.2fx improvement); %d shadow vertices (%.2fx improvement) in %.2f "
           "msec\n",
...
void meshlets(const Mesh &mesh, bool scan = false, bool uniform = false, bool flex = false,
              bool spatial = false, bool dump = false) {
    // NVidia recommends 64/126; we also test uniform configuration with 64/64 which is better for
    // earlier AMD GPUs
    const size_t max_vertices  = 64;
    const size_t max_triangles = uniform ? 64 : 126;
    const size_t min_triangles =
        spatial ? 16 : (uniform ? 24 : 32); // only used in flex/spatial modes
    ...
    double start = timestamp();
    size_t max_meshlets =
        meshopt_buildMeshletsBound(mesh.indices.size(), max_vertices, min_triangles);
    std::vector<meshopt_Meshlet> meshlets(max_meshlets);
    std::vector<unsigned int> meshlet_vertices(mesh.indices.size());
    std::vector<unsigned char> meshlet_triangles(mesh.indices.size());

    if (scan)
        meshlets.resize(meshopt_buildMeshletsScan(
            &meshlets[0], &meshlet_vertices[0], &meshlet_triangles[0], &mesh.indices[0],
            mesh.indices.size(), mesh.vertices.size(), max_vertices, max_triangles));
    else if (flex)
        meshlets.resize(meshopt_buildMeshletsFlex(
            &meshlets[0], &meshlet_vertices[0], &meshlet_triangles[0], &mesh.indices[0],
            mesh.indices.size(), &mesh.vertices[0].px, mesh.vertices.size(), sizeof(Vertex),
            max_vertices, min_triangles, max_triangles, cone_weight, split_factor));
    else if (spatial)
        meshlets.resize(meshopt_buildMeshletsSpatial(
            &meshlets[0], &meshlet_vertices[0], &meshlet_triangles[0], &mesh.indices[0],
            mesh.indices.size(), &mesh.vertices[0].px, mesh.vertices.size(), sizeof(Vertex),
            max_vertices, min_triangles, max_triangles, 0.f));
    else // note: equivalent to the call of buildMeshletsFlex() with split_factor = 0 and
         // min_triangles = max_triangles
    ...
    if (meshlets.size()) {
        ...
        // this is an example of how to trim the vertex/triangle arrays when copying data out to GPU
        // storage
    ...
    printf("Meshlets%c: %d meshlets (avg vertices %.1f, avg triangles %.1f, avg boundary %.1f, avg "
           "connected %.2f, not full %d) in %.2f msec\n",
           scan ? 'S' : (flex ? 'F' : (spatial ? 'X' : (uniform ? 'U' : ' '))),
           int(meshlets.size()), avg_vertices, avg_triangles, avg_boundary, avg_connected,
    ...
    for (size_t i = 0; i < meshlets.size(); ++i) {
        ...
        meshopt_Bounds bounds = meshopt_computeMeshletBounds(
            ...
            m.triangle_count, &mesh.vertices[0].px, mesh.vertices.size(), sizeof(Vertex));
        ...
        // perspective projection: dot(normalize(cone_apex - camera_position), cone_axis) >
        // cone_cutoff alternative formulation for perspective projection that doesn't use apex (and
        // uses cluster bounding sphere instead): dot(normalize(center - camera_position),
        // cone_axis) > cone_cutoff + radius / length(center - camera_position)
        float cview[3]    = {bounds.center[0] - camera[0], bounds.center[1] - camera[1],
                             bounds.center[2] - camera[2]};
        float cviewlength = sqrtf(cview[0] * cview[0] + cview[1] * cview[1] + cview[2] * cview[2]);

        rejected += cview[0] * bounds.cone_axis[0] + cview[1] * bounds.cone_axis[1] +
    ...
    printf("Bounds   : radius mean %f stddev %f; %.1f%% meshlets under 1σ; cone angle %.1f°; cone "
           "reject %.1f%% trivial accept %.1f%% in %.2f msec\n",
           radius_mean, radius_stddev, double(meshlets_std) / double(meshlets.size()) * 100,
           cone_mean, double(rejected) / double(meshlets.size()) * 100,
...
void spatialSort(const Mesh &mesh) {
    ...
    std::vector<unsigned char> vbuf(
        meshopt_encodeVertexBufferBound(mesh.vertices.size(), sizeof(PV)));
    vbuf.resize(meshopt_encodeVertexBuffer(&vbuf[0], vbuf.size(), &pv[0], mesh.vertices.size(),
                                           sizeof(PV)));
...
void spatialSortTriangles(const Mesh &mesh) {
    ...
    meshopt_spatialSortTriangles(&copy.indices[0], &copy.indices[0], mesh.indices.size(),
                                 &copy.vertices[0].px, copy.vertices.size(), sizeof(Vertex));
    ...
    std::vector<unsigned char> ibuf(
        meshopt_encodeIndexBufferBound(mesh.indices.size(), mesh.vertices.size()));
    ibuf.resize(
        meshopt_encodeIndexBuffer(&ibuf[0], ibuf.size(), &copy.indices[0], mesh.indices.size()));

    size_t csizev = compress(vbuf);
    size_t csizei = compress(ibuf);

    printf("SpatialT : %.1f bits/vertex (post-deflate %.1f bits/vertex); %.1f bits/triangle "
           "(post-deflate %.1f bits/triangle); sort %.2f msec\n",
           double(vbuf.size() * 8) / double(mesh.vertices.size()),
           double(csizev * 8) / double(mesh.vertices.size()),
           double(ibuf.size() * 8) / double(mesh.indices.size() / 3),
           double(csizei * 8) / double(mesh.indices.size() / 3), (end - start) * 1000);
}
...
void spatialClusterPoints(const Mesh &mesh, size_t cluster_size) {
    ...
    meshopt_spatialClusterPoints(&index[0], &mesh.vertices[0].px, mesh.vertices.size(),
                                 sizeof(Vertex), cluster_size);
    ...
    std::vector<unsigned char> vbuf(
        meshopt_encodeVertexBufferBound(mesh.vertices.size(), sizeof(PV)));
    vbuf.resize(meshopt_encodeVertexBuffer(&vbuf[0], vbuf.size(), &pvo[0], mesh.vertices.size(),
                                           sizeof(PV)));
...
void tessellationAdjacency(const Mesh &mesh) {
    ...
    meshopt_generateTessellationIndexBuffer(&tessib[0], &mesh.indices[0], mesh.indices.size(),
                                            &mesh.vertices[0].px, mesh.vertices.size(),
                                            sizeof(Vertex));
    ...
    meshopt_generateAdjacencyIndexBuffer(&adjib[0], &mesh.indices[0], mesh.indices.size(),
                                         &mesh.vertices[0].px, mesh.vertices.size(),
                                         sizeof(Vertex));
...
void provoking(const Mesh &mesh) {
    ...
    size_t pcount = meshopt_generateProvokingIndexBuffer(&pib[0], &reorder[0], &mesh.indices[0],
    ...
    // note: this is technically not promised by the interface (it may reorder triangles!), it just
    ...
    for (size_t i = 0; i < mesh.indices.size(); i += 3) {
        ...
        assert((a == ra && b == rb && c == rc) || (a == rb && b == rc && c == ra) ||
               (a == rc && b == ra && c == rb));
    }

    // best case number of vertices: max(vertex count, triangle count), assuming non-redundant
    // indexing (all vertices are used) note: this is a lower bound, and it's not theoretically
    // possible on some meshes; for example, a union of a flat shaded cube (12t 24v) and a smooth
    // shaded icosahedron (20t 12v) will have 36 vertices and 32 triangles however, the best case
    // for that union is 44 vertices (24 cube vertices + 20 icosahedron vertices due to provoking
    // invariant)
    size_t bestv = mesh.vertices.size() > mesh.indices.size() / 3 ? mesh.vertices.size()
                                                                  : mesh.indices.size() / 3;

    printf("Provoking: %d triangles / %d vertices (+%.1f%% extra) in %.2f msec\n",
           int(mesh.indices.size() / 3), int(pcount),
           double(pcount) / double(bestv) * 100.0 - 100.0, (end - start) * 1000);
}
...
void reindexFuzzy(const Mesh &mesh) {
    ...
    size_t up = meshopt_generateVertexRemap(&remap[0], &mesh.indices[0], mesh.indices.size(),
                                            &pv[0], mesh.vertices.size(), sizeof(PackedVertex));

    double middle = timestamp();

    size_t uf = meshopt_generateVertexRemapCustom(&remap[0], &mesh.indices[0], mesh.indices.size(),
                                                  &mesh.vertices[0].px, mesh.vertices.size(),
                                                  sizeof(Vertex), reindexCompare,
                                                  const_cast<Vertex *>(&mesh.vertices[0]));

    double end = timestamp();

    printf("ReindexQ : %d vertices => %d unique vertices in %.2f msec\n", int(mesh.vertices.size()),
           int(up), (middle - start) * 1000);
    printf("ReindexF : %d vertices => %d unique vertices in %.2f msec\n", int(mesh.vertices.size()),
           int(uf), (end - middle) * 1000);
}

void coverage(const Mesh &mesh) {
    double start = timestamp();

    meshopt_CoverageStatistics cs =
        meshopt_analyzeCoverage(&mesh.indices[0], mesh.indices.size(), &mesh.vertices[0].px,
                                mesh.vertices.size(), sizeof(Vertex));

    double end = timestamp();

    printf("Coverage : X %.1f%% Y %.1f%% Z %.1f%% in %.2f msec\n", cs.coverage[0] * 100,
           cs.coverage[1] * 100, cs.coverage[2] * 100, (end - start) * 1000);
}

void nanite(const std::vector<Vertex> &vertices,
            const std::vector<unsigned int> &indices); // nanite.cpp
...
void processDeinterleaved(const char *path) {
    // Most algorithms in the library work out of the box with deinterleaved geometry, but some
    // require slightly special treatment; this code runs a simplified version of complete opt.
    // pipeline using deinterleaved geo. There's no compression performed but you can trivially run
    // it by quantizing all elements and running meshopt_encodeVertexBuffer once for each vertex
    // stream.
    fastObjMesh *obj = fast_obj_read(path);
    if (!obj) {
        printf("Error loading %s: file not found\n", path);
    ...
    for (unsigned int i = 0; i < obj->face_count; ++i) {
        for (unsigned int j = 0; j < obj->face_vertices[i]; ++j) {
            ...
            // triangulate polygon on the fly; offset-3 is always the first polygon vertex
            if (j >= 3) {
                memcpy(&unindexed_pos[(vertex_offset + 0) * 3],
                       &unindexed_pos[(vertex_offset - 3) * 3], 3 * sizeof(float));
                memcpy(&unindexed_nrm[(vertex_offset + 0) * 3],
                       &unindexed_nrm[(vertex_offset - 3) * 3], 3 * sizeof(float));
                memcpy(&unindexed_uv[(vertex_offset + 0) * 2],
                       &unindexed_uv[(vertex_offset - 3) * 2], 2 * sizeof(float));
                memcpy(&unindexed_pos[(vertex_offset + 1) * 3],
                       &unindexed_pos[(vertex_offset - 1) * 3], 3 * sizeof(float));
                memcpy(&unindexed_nrm[(vertex_offset + 1) * 3],
                       &unindexed_nrm[(vertex_offset - 1) * 3], 3 * sizeof(float));
                memcpy(&unindexed_uv[(vertex_offset + 1) * 2],
                       &unindexed_uv[(vertex_offset - 1) * 2], 2 * sizeof(float));
                vertex_offset += 2;
            }
    ...
    size_t total_vertices =
        meshopt_generateVertexRemapMulti(&remap[0], NULL, total_indices, total_indices, streams,
    ...
    meshopt_remapVertexBuffer(&pos[0], &unindexed_pos[0], total_indices, sizeof(float) * 3,
                              &remap[0]);

    std::vector<float> nrm(total_vertices * 3);
    meshopt_remapVertexBuffer(&nrm[0], &unindexed_nrm[0], total_indices, sizeof(float) * 3,
                              &remap[0]);

    std::vector<float> uv(total_vertices * 2);
    meshopt_remapVertexBuffer(&uv[0], &unindexed_uv[0], total_indices, sizeof(float) * 2,
                              &remap[0]);
    ...
    // note: since shadow index buffer is computed based on regular vertex/index buffer, the stream
    // points at the indexed data - not unindexed_pos
    meshopt_Stream shadow_stream = {&pos[0], sizeof(float) * 3, sizeof(float) * 3};

    std::vector<unsigned int> shadow_indices(total_indices);
    meshopt_generateShadowIndexBufferMulti(&shadow_indices[0], &indices[0], total_indices,
                                           total_vertices, &shadow_stream, 1);

    meshopt_optimizeVertexCache(&shadow_indices[0], &shadow_indices[0], total_indices,
                                total_vertices);

    double shadow = timestamp();

    printf("Deintrlvd: %d vertices, reindexed in %.2f msec, optimized in %.2f msec, generated & "
           "optimized shadow indices in %.2f msec\n",
           int(total_vertices), (reindex - start) * 1000, (optimize - reindex) * 1000,
           (shadow - optimize) * 1000);
}
...
void process(const char *path) {
    ...
    Mesh copystrip = mesh;
    meshopt_optimizeVertexCacheStrip(&copystrip.indices[0], &copystrip.indices[0],
                                     copystrip.indices.size(), copystrip.vertices.size());
    meshopt_optimizeVertexFetch(&copystrip.vertices[0], &copystrip.indices[0],
                                copystrip.indices.size(), &copystrip.vertices[0],
                                copystrip.vertices.size(), sizeof(Vertex));
    ...
    strip.resize(meshopt_stripify(&strip[0], &copystrip.indices[0], copystrip.indices.size(),
                                  copystrip.vertices.size(), 0));
